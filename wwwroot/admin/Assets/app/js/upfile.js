var UpfilePage = {
    ...
    onEvent: function () {
        ...
        jQuery(document).on("click", ".attachFile", function () {
            ...
            jQuery(data.rel).attr("data-check-file", data.checkFile);
            jQuery(data.rel).attr("data-messages", data.messages);
        ...
        jQuery(document).on("click", ".syncFolders", function () {
            var data = jQuery(this).getData();
            jQuery.ajax({
                type: "POST",
                url: "http://localhost:81/plugin/sync",
                data: JSON.stringify({
            ...
            if (!obj.hasClass("setUpFiled"))
            {
                ...
                obj.ajaxUploader({
                    ...
                    onSuccess: function (e, file, t, data) {
                        ...
                        if (Utils.notEmpty(dataObj.checkFile) && dataObj.checkFile == "image") {
                            if (!Utils.IsImage(Utils.getExtension(data.FileName))) {
                                    Utils.setError(dataObj.messages)
                                    return;
                                }
                        }
