var Utils = {
    ...
    autoCloseAllFlash: function (delay) {
        if (delay == undefined)
            delay = 1000;
        var flashCount = 0;
        jQuery(".flash").each(function () {
            flashCount++;
            jQuery(this).delay(flashCount * delay).fadeOut("fast");
        });
    },
    autoResize: function () {
        try {
    ...
    updateSlider: function (container) {
        try {
            if (!container)
                container = $(document);

            container.find(".useSlickSlider").each(function () {
                var id = $(this).attr("id");
                if (!id) {
    ...
    CallAjax: function (data, url, callbackSuccess) {
        jQuery.ajax({
            type: "POST",
            async: true,
            url: url,
