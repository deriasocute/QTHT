    ...
    upEvent: function (container) {
        ...
        container.find(".morris-bar-chart").each(function() {
            ...
            for(i=0;i<datas.labels.length;i++){
                ...
                series.push({
                    'name': datas.labels[i],
                    'color': datas.labels[i]=="Thực tế" ? '#F7B300' : '#fff',
                    'data': dataNew
                });
            }
            newData.push({
        ...
        container.find(".morris-bar-chart-old").each(function () {
            ...
            try {
                ...
                var chart = Morris.Bar({
                    ...
                    barColors: mrdata.barColors,
                    gridTextColor: "#fff"
                });
                var legendItem = "";
                chart.options.labels.forEach(function (label, i) {
                    legendItem += '<div class="Legend_item"><span style="display:inline-block;width:20px;background-color:' + chart.options.barColors[i] + ';">&nbsp;</span> ' + label + '</div>'

                });
                if (jQuery(this).next(".DocproChart_legend").length != 0) { jQuery(this).next(".DocproChart_legend").remove(); }
                obj.after('<div class="DocproChart_legend">' + legendItem + '</div>');
                if (obj.hasClass('usePercentColor')) {
                    var actuals_color = "#a0d468";
                    var percent = 0;
                    var count = 0;
                    for (var i = 0; i < mrdata.data.length; i++) {
                        percent = Math.round(mrdata.data[i].now / mrdata.data[i].plan * 100, 3);
                        if (percent < 30) {
                        ...
                        obj.children('svg').children("rect:nth-of-type(" + count + ")").css("fill", actuals_color);
                    }
                }
    ...
    onEvent: function () {
        ...
        jQuery(document).on("click", ".quickDeletes, .quickConfirms", function () {
            try {
                ...
                var table = jQuery(this).closest(".dataTables_wrapper").find("table, div.table");
